...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	u8 buffer[2] = {reg+2, val};
	if (XSpi_Transfer(&SpiInstance, buffer, NULL, 2) != 0)
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 buffer[nbytes+1];
	buffer[0] = reg+2;
	for (int i=1; i<=nbytes; i++) buffer[i] = data[i-1];
	if (XSpi_Transfer(&SpiInstance, buffer, NULL, nbytes+1) != 0)
...
BYTE MAXreg_rd(BYTE reg) {
	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	u8 buffer[2] = {reg, 0};
	if (XSpi_Transfer(&SpiInstance, buffer, buffer, 2) != 0)
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 buffer[nbytes+1];
	buffer[0] = reg;
	if (XSpi_Transfer(&SpiInstance, buffer, buffer, nbytes+1) != 0)
		xil_printf ("MAXbytes_rd Error");
	for (int i=1; i<=nbytes; i++) data[i-1] = buffer[i];
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
